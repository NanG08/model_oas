...
from object_detection_v8s import extract_objects_info
...
class FusionLogger:
    ...
    def log_fusion_result(self, objects, action, speech, scene_context, confidence_scores):
        ...
        speech_str = speech['text'] if speech['text'] != None else "None"
...
class FusionCompanion:
    ...
    def setup_models(self):
        ...
        try:
            ...
            self.action_model = load_action_model("best_model_quantized.pt", device=self.device, num_classes=5, hidden_size=128)
    ...
    def process_frame(self, frame,external_speech_text=None):
        self.frame_count += 1
        results = {
        'objects': [],
        'action': {'text': None, 'confidence': 0.0},
        'speech': {'text': external_speech_text if external_speech_text else None, 'context': []}
    }
